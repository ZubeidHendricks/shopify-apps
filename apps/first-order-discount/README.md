...
Give a percentage off to first-time customers (no prior orders). Starter targets logged-in new customers; Pro can also include guest checkouts.

Part of the [Shopify app factory](../../README.md). Built from the base template
+ the `first-order` preset, so OAuth, billing, GDPR webhooks, settings, and
