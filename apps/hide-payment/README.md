...
Hide a named payment method when the cart total is below a threshold, and (Pro) for specific countries.

Part of the [Shopify app factory](../../README.md). Built from the base template
+ the `payment-hide` preset, so OAuth, billing, GDPR webhooks, settings, and
analytics are all inherited from `@factory/core`.

| | |
| --- | --- |
| Kind | `payment-hide` |
| Function target | `purchase.payment-customization.run` |
| Access scopes | `write_payment_customizations` |
