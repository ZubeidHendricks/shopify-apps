...
Enforce a minimum order value and (Pro) a maximum item count, blocking checkout when the cart violates the rule.

Part of the [Shopify app factory](../../README.md). Built from the base template
+ the `cart-min-max` preset, so OAuth, billing, GDPR webhooks, settings, and
