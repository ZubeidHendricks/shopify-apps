...
Rename a delivery option at checkout (e.g. 'Standard' -> 'Free Shipping (3-5 days)') and, on Pro, move it to the top of the list.

Part of the [Shopify app factory](../../README.md). Built from the base template
+ the `rename-shipping` preset, so OAuth, billing, GDPR webhooks, settings, and
analytics are all inherited from `@factory/core`.

| | |
| --- | --- |
| Kind | `rename-shipping` |
| Function target | `purchase.delivery-customization.run` |
| Access scopes | `write_delivery_customizations` |
