...
Free shipping once the cart subtotal crosses a threshold. Starter uses one threshold; Pro adds a second, higher threshold for premium tiers.

Part of the [Shopify app factory](../../README.md). Built from the base template
+ the `free-shipping` preset, so OAuth, billing, GDPR webhooks, settings, and
analytics are all inherited from `@factory/core`.

| | |
| --- | --- |
| Kind | `free-shipping` |
| Function target | `cart.delivery-options.discounts.generate.run` |
