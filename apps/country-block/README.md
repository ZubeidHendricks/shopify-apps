...
Block checkout for shipping addresses in certain countries. Starter uses a blocklist; Pro switches to allowlist mode (only listed countries can check out).

Part of the [Shopify app factory](../../README.md). Built from the base template
+ the `country-block` preset, so OAuth, billing, GDPR webhooks, settings, and
