/**
 * Spend-more-save-more config screen. Reads billing tier, reads/writes the JSON
 * config the Function consumes, gates multi-tier behind Pro.
 */
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const config: Config = raw ? JSON.parse(raw) : { plan: tier, tiers: [{ threshold: 50, percentage: 10 }] };
  config.plan = tier;
      ...
      <BlockStack gap="400">
        <Card>
          <BlockStack gap="200">
            ...
            {!isPro && (
              <Banner tone="info" title="Unlock multiple spend tiers">
          ...
          <BlockStack gap="300">
            {tiers.map((t, i) => (
              <BlockStack key={i} gap="100">
                <TextField label={`Tier ${i + 1}: minimum order value`} type="number" value={String(t.threshold)}
                  onChange={(v) => { const n = [...tiers]; n[i] = { ...n[i], threshold: Number(v) }; setTiers(n); }} autoComplete="off" />
                <TextField label="% off" type="number" value={String(t.percentage)}
                  onChange={(v) => { const n = [...tiers]; n[i] = { ...n[i], percentage: Number(v) }; setTiers(n); }} autoComplete="off" />
              </BlockStack>
            ))}
            {isPro && <Button onClick={() => setTiers([...tiers, { threshold: 100, percentage: 15 }])}>Add tier</Button>}
